...
public static class MAST_Placement_PaintArea
{
    ...
    public static void StartPaintArea()
    {
        if (MAST_Placement_Visualizer.GetGameObject() != null)
        {
            ...
            paintAreaStart.y = MAST_Settings.gui.grid.gridHeight *
                MAST_Settings.gui.grid.unitSize + MAST_Const.grid.yOffsetToAvoidTearing;
    ...
    public static void CompletePaintArea()
    {
        ...
        else
        {
            ...
            int xCount = (int)(Mathf.Abs(paintAreaStart.x - paintAreaEnd.x) / MAST_Settings.gui.grid.unitSize);
            int zCount = (int)(Mathf.Abs(paintAreaStart.z - paintAreaEnd.z) / MAST_Settings.gui.grid.unitSize);
            ...
            for (int x = 0; x <= xCount; x++)
            {
                for (int z = 0; z <= zCount; z++)
                {
                    ...
                    MAST_Placement_Visualizer.GetGameObject().transform.position =
                        new Vector3(xBase + (x * MAST_Settings.gui.grid.unitSize),
                        MAST_Settings.gui.grid.gridHeight * MAST_Settings.gui.grid.unitSize,
                        zBase + (z * MAST_Settings.gui.grid.unitSize));
