...
// ---------------------------------------------------------------------------
    ...
    public static Vector3 GetPositionOnGridClosestToMousePointer()
    {
        ...
        return new Vector3(xPos, MAST_Settings.gui.grid.gridHeight * MAST_Settings.gui.grid.unitSize, zPos);
    }
    ...
    private static float RoundToNearestGridCenter(float positionOnAxis)
    {
        return Mathf.Round(
            ...
            + (MAST_Settings.gui.grid.unitSize / 2))
            * MAST_Settings.gui.grid.unitSize
            - (MAST_Settings.gui.grid.unitSize / 2);
    }
#endregion
...
// ---------------------------------------------------------------------------
    ...
    public class Randomizer
    {
        ...
        public class Rotation
        {
            ...
            {
                try { return mastScript.randomizer.rotateX; }
                catch { return MAST_Settings.placement.randomizer.rotateX; }
            }
            public static bool GetY()
            {
                try { return mastScript.randomizer.rotateY; }
                catch { return MAST_Settings.placement.randomizer.rotateY; }
            }
            public static bool GetZ()
            {
                try { return mastScript.randomizer.rotateZ; }
                catch { return MAST_Settings.placement.randomizer.rotateZ; }
            }
        }
