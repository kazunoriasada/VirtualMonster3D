...
public static class MAST_Asset_Loader
{
    ...
    // ---------------------------------------------------------------------------
    public static GameObject[] GetPrefabsInSelectedFolder(bool includeSubfolders)
    {
        // Get path of currently selected folder
        string pathToSelectedFolder = GetPathOfSelectedFolder();
        
        // Get the GUID of every file in that folder that is of the file type prefab
        string[] GUIDOfAllPrefabsInFolder =
            AssetDatabase.FindAssets("t:prefab", new[] { pathToSelectedFolder });
        
        // Create array to store the gameObjects
        GameObject[] allPrefabsAtPath =
            new GameObject[GUIDOfAllPrefabsInFolder.Length];
